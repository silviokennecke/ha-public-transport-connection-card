import{LitElement,html,css}from"https://unpkg.com/lit-element@2.0.1/lit-element.js?module";function timeToStr(time){const parse=Date.parse(time);return parse?new Date(parse).toLocaleTimeString([],{timeStyle:"short"}):time}function delayToMinutes(delay){if(typeof delay==="number"){return delay}if(typeof delay==="string"&&delay.includes(":")){const delayParts=delay.split(":");const hours=parseInt(delayParts[0])||0;const minutes=parseInt(delayParts[1])||0;return hours*60+minutes}return parseInt(delay)||0}class PublicTransportConnectionCard extends LitElement{static getConfigForm(){return{schema:[{name:"entity",required:true,selector:{entity:{domain:"sensor"}}},{name:"title",selector:{text:{}}},{name:"icon",selector:{icon:{}}},{name:"departure_station",selector:{text:{}}},{name:"arrival_station",selector:{text:{}}},{name:"connections_attribute",required:true,selector:{attribute:{entity_id:""}},context:{filter_entity:"entity"}},{name:"connection_properties",type:"grid",schema:[{name:"description",selector:{text:{}}},{name:"departure_time",selector:{text:{}}},{name:"departure_delay",selector:{text:{}}},{name:"arrival_time",selector:{text:{}}},{name:"arrival_delay",selector:{text:{}}}]},{name:"displayed_connections",selector:{number:{min:1}}},{name:"theme",selector:{select:{options:["deutsche-bahn","homeassistant"],custom_value:true}}}]}}static getStubConfig(hass,unusedEntities,allEntities){const defaultAttributes={connections:["departures","connections"],departureStation:["start","origin"],arrivalStation:["goal","destination"]};function getAttributeName(entityId,defaultAttributes){const entity=hass.states[entityId]??{attributes:{}};for(const attribute of defaultAttributes){if(entity.attributes[attribute]!==undefined){return attribute}}return undefined}function getAttribute(entityId,defaultAttributes,defaultValue=undefined){const entity=hass.states[entityId]??{attributes:{}};const attributeName=getAttributeName(entityId,defaultAttributes);if(attributeName===undefined){return defaultValue}else{return entity.attributes[attributeName]}}function isPublicTransportSensor(entityId){if(entityId.split(".")[0]!=="sensor"){return false}return getAttributeName(entityId,defaultAttributes.connections)!==undefined}let entityId=unusedEntities.find(isPublicTransportSensor);if(!entityId){entityId=allEntities.find(isPublicTransportSensor)||""}return{title:"",departure_station:getAttribute(entityId,defaultAttributes.departureStation,""),arrival_station:getAttribute(entityId,defaultAttributes.arrivalStation,""),entity:entityId,connections_attribute:getAttributeName(entityId,defaultAttributes.connections),connection_properties:{description:"products",departure_time:"departure",departure_delay:"delay",arrival_time:"arrival",arrival_delay:"delay_arrival"},displayed_connections:3,theme:"deutsche-bahn"}}static get properties(){return{hass:{},config:{}}}render(){const title=this.config.title;const entity=this.config.entity;const theme=this.config.theme;const stateObj=this.hass.states[entity];if(!stateObj){return html`
        ...
        `}}customElements.define("public-transport-connection-card",PublicTransportConnectionCard);window.customCards=window.customCards||[];window.customCards.push({type:"public-transport-connection-card",name:"Public Transport Connection Card",preview:true,description:"Display your next connections via public transportation.",documentationURL:"https://github.com/silviokennecke/ha-public-transport-connection-card/wiki/Components#public-transport-connection"});